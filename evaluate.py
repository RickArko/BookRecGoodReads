...
def batched_neighbors(
    knn: NearestNeighbors,
    queries: csr_matrix,
    n_neighbors: int,
    seeds: list[int],
) -> list[list[int]]:
    """Run a batched ``kneighbors`` call and drop self-matches per row.

    Each row's seed is removed (if present) and the remainder is truncated to
    ``n_neighbors``. This avoids assuming the self-match is always at column 0.
    """
    distances, indices = knn.kneighbors(queries, n_neighbors=n_neighbors + 1)
    out: list[list[int]] = []
    for row, seed in zip(indices, seeds):
        filtered = [int(idx) for idx in row if int(idx) != int(seed)][:n_neighbors]
        out.append(filtered)
    return out


def query_collaborative(
    knn: NearestNeighbors, interactions: csr_matrix, samples: list[HoldoutSample], n_neighbors: int
) -> list[list[int]]:
    seed_indices = [s.seed_book_idx for s in samples]
    queries = interactions[seed_indices]
    return batched_neighbors(knn, queries, n_neighbors, seed_indices)
...
def query_content(
    ...
    """Content KNN where input + output indices are *collab indices*.

    Books missing content features get an empty rec list (no metadata signal).
    """
    valid_idxs = []
    valid_seed_collab = []
    ...
    out: list[list[int]] = [[] for _ in samples]
    if not valid_idxs:
        return out
