...
book_user_mat = dff.pivot(index="book_id", columns="user_id", values="rating").fillna(0)

book_to_idx = {book: i for i, book in enumerate(list(dftmp.loc[book_user_mat.index].title))}
...
start = time.time()
book_user_matrix = df_filter.pivot(index="book_id", columns="user_id", values="rating").fillna(0)
...
start = time.time()
book_user_matrix = dfq.pivot(index="book_id", columns="user_id", values="rating").fillna(0)
...
book_to_idx = {book: i for i, book in enumerate(list(df_books.set_index("book_id").loc[book_user_mat.index].title))}
